...
pub const Error = cuda.Error || runtime.Error || export_.Error || jit.Error || error{
...
pub const KernelBinding = struct {
    ...
    pub fn init(prefix: []const u8, function_name: []const u8) Error!KernelBinding {
        _ = try runtime.RuntimeSymbols.init(prefix, function_name);
        return .{
            .prefix = prefix,
    ...
    pub fn writeCInterfaceSymbol(self: KernelBinding, out: anytype) Error!void {
        const symbols = try runtime.RuntimeSymbols.init(
            self.prefix,
            self.public_name,
        );
...
pub const ExecutableKernel = struct {
    compile_plan: *const runtime.CompilePlan,
    launch_plan: *const runtime.LaunchPlan,
...
pub fn writeExecutionCWrapper(
    ...
    try out.append("\n");
    try runtime.writeCInterfaceWrapperSource(
        out,
...
pub fn makeExecutableKernel(
    plan: *const runtime.CompilePlan,
    launch: *const runtime.LaunchPlan,
    mlir_path: []const u8,
    cubin_path: []const u8,
...
test "execution: executable kernel manifest includes compile and launch wiring" {
    var args: runtime.ArgPack = .{};
    try args.append(try runtime.PackedArg.scalarBytes("alpha", .scalar_u64, "12345678"));
    ...
    const symbols = try runtime.RuntimeSymbols.init("notcute", "kernel");
    const launch: runtime.LaunchPlan = .{
        .symbols = symbols,
        .module = try runtime.BinaryModule.init("kernel.cubin", .cubin),
        .config = cfg,
        .args = args,
    };
    const compile: runtime.CompilePlan = .{
...
test "execution: runbook documents CUDA driver launch sequence" {
    ...
    const symbols = try runtime.RuntimeSymbols.init("demo", "copy_kernel");
    const launch: runtime.LaunchPlan = .{
        .symbols = symbols,
