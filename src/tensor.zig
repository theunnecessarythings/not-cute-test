...
const layout_algebra = @import("layout.zig");
const layout_core = @import("layout.zig");
...
pub const Selector = layout.Selector;
pub const keep = layout.keep;
...
pub const BasicTensorSsa = struct {
    value: mlir.Value,
    ...
    pub fn toVector(self: BasicTensorSsa) mlir.Value {
        return self.value;
    }
    ...
    pub fn broadcastTo(
        ...
        return BasicTensorSsa.init(result, result_shape, self.dtype);
    }
    ...
    pub fn applyUnary(self: BasicTensorSsa, builder: anytype, op: BasicUnaryOp) Error!BasicTensorSsa {
        var vt: mlir.TextBuffer(128) = .{};
        ...
        return BasicTensorSsa.init(value, self.shape_value, self.dtype);
    }
    ...
    pub fn reshape(
        ...
        return BasicTensorSsa.init(v, new_shape, self.dtype);
    }

    pub fn reduce(self: BasicTensorSsa, builder: anytype, op: BasicReduceOp) Error!mlir.Value {
        var vt: mlir.TextBuffer(128) = .{};
        try self.vectorType(&vt);
        return builder.genericOp(
            op.mlirName(),
...
pub const BasicBinaryOp = enum {
    ...
    pub fn mlirName(self: BasicBinaryOp, dtype: typing.Numeric) []const u8 {
        return switch (self) {
    ...
    pub fn attrs(self: BasicBinaryOp) []const mlir.Attribute {
        return switch (self) {
...
pub const BasicUnaryOp = enum {
    neg,
    abs,

    pub fn mlirName(self: BasicUnaryOp, dtype: typing.Numeric) []const u8 {
        return switch (self) {
...
pub const BasicReduceOp = enum {
    ...
    pub fn mlirName(self: BasicReduceOp) []const u8 {
        return switch (self) {
...
pub fn makeBasicIdentityTensor(shape: layout.Tree) Error!Tensor {
    const l = try layout.Layout.makeCompact(shape);
    return Tensor.init(.{ .identity = {} }, l, typing.Int32, .generic);
}

pub fn makeBasicRmemTensor(dtype: typing.Numeric, shape: layout.Tree) Error!Tensor {
    const l = try layout.Layout.makeCompact(shape);
    return Tensor.init(.{ .rmem = {} }, l, dtype, .generic);
}

pub fn makeBasicFragment(dtype: typing.Numeric, shape: layout.Tree) Error!Tensor {
    const l = try layout.Layout.makeCompact(shape);
    return Tensor.init(.{ .fragment = {} }, l, dtype, .generic);
}

pub fn makeBasicFragmentLike(source: Tensor) Error!Tensor {
    return makeBasicFragment(source.dtype, source.layout_value.shape);
}

pub fn makeBasicRmemTensorLike(source: Tensor) Error!Tensor {
    return makeBasicRmemTensor(source.dtype, source.layout_value.shape);
}

pub fn recastBasicTensor(source: Tensor, dtype: typing.Numeric) Error!Tensor {
    return source.recast(dtype);
}

pub fn basicDomainOffset(source: Tensor, coord: layout.Tree) Error!Tensor {
...
pub fn basicFull(
    ...
    return BasicTensorSsa.init(v, shape, dtype);
}

pub fn basicZerosLike(builder: anytype, source: BasicTensorSsa) Error!BasicTensorSsa {
    const zero = try builder.constantI(0, mlir.Type.raw(source.dtype.mlir_type));
    return basicFull(builder, source.shape_value, source.dtype, .{ .value = zero });
}

pub fn basicOnesLike(builder: anytype, source: BasicTensorSsa) Error!BasicTensorSsa {
    const one = try builder.constantI(1, mlir.Type.raw(source.dtype.mlir_type));
    return basicFull(builder, source.shape_value, source.dtype, .{ .value = one });
}

pub fn basicWhere(
    builder: anytype,
    cond: BasicTensorSsa,
    if_value: BasicTensorSsa,
    else_value: BasicTensorSsa,
) Error!BasicTensorSsa {
    ...
    return BasicTensorSsa.init(v, shape, a.dtype);
}

pub fn basicGather(builder: anytype, source: BasicTensorSsa, indices: BasicTensorSsa) Error!BasicTensorSsa {
    ...
    return BasicTensorSsa.init(v, source.shape_value, source.dtype);
}

pub fn basicScatter(
    builder: anytype,
    source: BasicTensorSsa,
    indices: BasicTensorSsa,
    values: BasicTensorSsa,
) Error!BasicTensorSsa {
    var src_ty: mlir.TextBuffer(128) = .{};
    var idx_ty: mlir.TextBuffer(128) = .{};
...
test "tensor: make tensor and domain offset" {
    ...
    const t = makeBasicTensor(.{ .pointer = ptr }, l, typing.Float32, .gmem);
...
test "tensor: TensorSSA emits binary and reduction operations" {
    ...
    const lhs = try basicFull(&b, shape, typing.Int32, .{ .value = c0 });
    const rhs = try basicOnesLike(&b, lhs);
...
pub const TensorMeta = struct {
    ...
    pub fn fromTensor(source: Tensor) TensorMeta {
        return .{
    ...
    pub fn toTensor(self: TensorMeta) Tensor {
        return Tensor.init(
            self.engine,
            self.layout_value,
            self.dtype,
    ...
    pub fn cutlassTensorTypeText(self: TensorMeta, out: anytype) Error!void {
        try cutlass.writeMemRefTypeForLayout(
            out,
            self.dtype,
            self.memspace,
            self.dtype.bytes(),
...
pub const TensorValue = struct {
    ...
    pub fn access(
        ...
        return switch (try self.meta.access(selector)) {
            ...
            .element => |elt| blk: {
                ...
                const coord = try cutlass.makeCoordFromScalar(builder, elt.offset);
                const result = try cutlass.emitMemrefLoad(
                    builder,
                    self.value,
                    coord.value,
    ...
    pub fn load(
        ...
        var memref_ty_buf: mlir.TextBuffer(512) = .{};
        try self.meta.cutlassTensorTypeText(&memref_ty_buf);
        const raw_value = try cutlass.emitMemrefLoadVec(
            builder,
            self.value,
            mlir.Type.raw(memref_ty_buf.slice()),
    ...
    pub fn store(
        ...
        var memref_ty_buf: mlir.TextBuffer(512) = .{};
        try self.meta.cutlassTensorTypeText(&memref_ty_buf);
        try cutlass.emitMemrefStoreVec(
            builder,
            casted.value,
            self.value,
...
fn ssa_inferBroadcastShape(lhs: Tree, rhs: Tree) Error!Tree {
    if (lhs.equals(&rhs)) return lhs;
    const lprod = try lhs.product();
    const rprod = try rhs.product();
    if (lprod == 1) return rhs;
...
pub const TensorSSA = SsaTensor;
pub const TensorSsa = SsaTensor;

pub fn api_makeTensor(meta: TensorMeta, value: mlir.Value) Error!TensorValue {
    return TensorValue.initFromMeta(meta, value);
}
pub fn api_makeIdentityTensor(builder: anytype, shape: layout.Tree) Error!TensorSSA {
    return TensorSSA.empty(builder, shape, typing.Int32);
}
pub fn api_makeRmemTensor(
    builder: anytype,
    shape: layout.Tree,
    dtype: typing.Numeric,
) Error!TensorSSA {
...
pub fn api_recastTensor(source: Tensor, dtype: typing.Numeric) Error!Tensor {
    return source.recast(dtype);
}
pub fn api_domainOffset(source: Tensor, coord: layout.Tree) Error!Tensor {
    return source.domainOffset(coord);
}
pub fn api_fullLike(
    builder: anytype,
...
pub fn api_onesLike(builder: anytype, source: TensorSSA) Error!TensorSSA {
    return TensorSSA.ones(builder, source.shape_value, source.dtype);
}
pub fn api_zerosLike(builder: anytype, source: TensorSSA) Error!TensorSSA {
    return TensorSSA.zeros(builder, source.shape_value, source.dtype);
}
pub fn api_any_(source: TensorSSA, builder: anytype) Error!SsaValue {
    return source.reduceAll(builder, .any);
}
pub fn api_all_(source: TensorSSA, builder: anytype) Error!SsaValue {
    return source.reduceAll(builder, .all);
}
pub fn api_printTensor(builder: anytype, source: TensorValue) Error!void {
    try builder.operationNoResult(.{
...
test "tensor_api: source-name constructors wrap tensor SSA" {
    var b: mlir.Builder(4096) = .{};
    const t = try api_makeRmemTensor(
        &b,
        layout.Tree.fromComptime(.{ 2, 2 }),
        typing.Float32,
    );
    const z = try api_zerosLike(&b, t);
