...
pub const Shape = struct {
    ...
    pub fn rowMajorStrides(self: Shape) Error!Stride {
        ...
        while (i > 0) {
            ...
            running = std.math.mul(Scalar, running, @intCast(v)) catch return Error.Overflow;
        }
        return out;
    }
...
pub const LayoutView = struct {
    ...
    pub fn linearize(self: LayoutView, coord: Coord) Error!Unsigned {
        ...
        var offset: Scalar = 0;
        for (0..coord.len) |i| {
            const term = std.math.mul(Scalar, @intCast(coord.values[i]), self.stride.values[i]) catch return Error.Overflow;
    ...
    pub fn subView(self: LayoutView, fixed_prefix: []const Unsigned) Error!SubView {
        ...
        for (fixed_prefix, 0..) |v, i| {
            ...
            const term = std.math.mul(Scalar, @intCast(v), self.stride.values[i]) catch return Error.Overflow;
            offset = std.math.add(Scalar, offset, term) catch return Error.Overflow;
        }
        ...
        return .{
            ...
            .view = try LayoutView.init(try Shape.init(new_shape[0..len]), try Stride.init(new_stride[0..len])),
        };
    }
};
...
test "semantics: broadcast and tile shape handle edge cases" {
    ...
    const tiled = try tileShape(try Shape.staticInts(&.{ 17, 33 }), try Shape.staticInts(&.{ 8, 16 }));
...
pub fn isCompactRowMajor(view: LayoutView) Error!bool {
    ...
    return std.mem.eql(Scalar, view.stride.values[0..view.stride.len], expected.values[0..expected.len]);
}
...
pub fn cosize(view: LayoutView) Error!Unsigned {
    ...
    for (0..view.shape.len) |i| {
        ...
        const term = std.math.mul(Scalar, @intCast(extent - 1), view.stride.values[i]) catch return Error.Overflow;
...
pub fn splitCoordByTile(coord: Coord, tile: Shape) Error!TiledIndex {
    ...
    return .{ .tile_coord = try Coord.init(tile_values[0..coord.len]), .residue_coord = try Coord.init(residue_values[0..coord.len]) };
}
...
pub fn localTile(view: LayoutView, tile_shape: Shape, tile_coord: Coord) Error!LocalTile {
    if (view.shape.len != tile_shape.len or tile_coord.len != tile_shape.len) return Error.InvalidRank;
    ...
    for (0..tile_shape.len) |i| {
        ...
        const coord = std.math.mul(Unsigned, tile_coord.values[i], tile_extent) catch return Error.Overflow;
        const term = std.math.mul(Scalar, @intCast(coord), view.stride.values[i]) catch return Error.Overflow;
        base = std.math.add(Scalar, base, term) catch return Error.Overflow;
    }
    if (base < 0) return Error.InvalidCoordinate;
    return .{ .tile_shape = tile_shape, .tile_count_shape = counts, .tile_coord = tile_coord, .base_offset = @intCast(base) };
}
